...
export default function Messages() {
  ...
  useEffect(() => {
    ...
    const newChannel = supabase.channel(`room:${selectedChannel}`);
    ...
    newChannel
      .on('presence', { event: 'sync' }, () => {
        ...
        for (const id in state) {
          // @ts-ignore
          if (state[id][0]?.isTyping) typing.add(state[id][0].user);
        }
  ...
  const handleTyping = () => {
    if (!channel || !selectedChannel || !user) return;

    channel.track({ user: user.email || user.id, isTyping: true });
    ...
    typingTimeoutRef.current = setTimeout(() => {
      channel.track({ user: user.email || user.id, isTyping: false });
    }, 2000);
  };
